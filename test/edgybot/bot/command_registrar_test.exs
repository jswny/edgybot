defmodule Edgybot.Bot.CommandRegistrarTest do
  ...
  describe "list_command_definitions/1" do
    ...
    test "does not list duplicate command definitions" do
      ...
      assert 1 =
               Enum.count(command_definitions, fn command_definitions ->
                 command_definitions.name == TestCommand.command_name_1()
               end)
    end
  end
  ...
  describe "load_command_module/1" do
    @tag :skip_default_command
    test "loads command module with multiple definitions" do
      CommandRegistrar.load_command_module(TestCommand)

      assert TestCommand = CommandRegistrar.get_command_module(TestCommand.command_name_1())
      assert TestCommand = CommandRegistrar.get_command_module(TestCommand.command_name_2())
    end
  end
end
