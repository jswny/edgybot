defmodule Edgybot.Bot.Plugin.IndexPlugin do
  ...
  def handle_interaction(
    ...
    case Qdrant.embed_and_find_closest(points_collection, query, limit, score_threshold) do
      {:ok, response} ->
  ...
  defp format_search_results(%{"result" => result}) when is_list(result) do
    Enum.map_join(result, "\n", fn %{
      ...
      "- ID: #{id_value}, score: #{score_value}, user: #{user_id_value}, channel: #{channel_id_value} #{Designer.code_block(content)}"
    end)
  end
end
